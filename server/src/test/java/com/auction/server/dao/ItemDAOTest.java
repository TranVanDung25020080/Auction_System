...
class ItemDAOTest {
    ...
    void setUp() throws SQLException {
        ...
        String fixNullStatusSql = "UPDATE item SET item_status = 'AVAILABLE' WHERE item_status IS NULL";
        try (Connection conn = MyDatabaseConfig.getConnection();
             PreparedStatement pst = conn.prepareStatement(fixNullStatusSql)) {
            int rowsFixed = pst.executeUpdate();
            if (rowsFixed > 0) {
                System.out.println("-> [Đã sửa lỗi] Tìm thấy và xử lý " + rowsFixed + " bản ghi bị NULL trạng thái!");
            }
        }
    ...
    void testGetAllItems_Success() {
        assertDoesNotThrow(() -> {
            ...
            System.out.println("-> Thực tế: Đã tải thành công và ánh xạ an toàn thông qua Reflection!");
            System.out.println("Tổng số sản phẩm hiện tại trong DB: " + allItems.size());

            if (!allItems.isEmpty()) {
                assertNotNull(allItems.getFirst().getName(), "Lỗi dữ liệu: Tên của bản ghi đầu tiên trống!");
                System.out.println("Bản ghi kiểm tra đầu tiên trong DB: " + allItems.getFirst().getName());
            }
        });
    }
