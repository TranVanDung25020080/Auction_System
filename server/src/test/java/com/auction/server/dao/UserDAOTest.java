...
class UserDAOTest {
    ...
    private void cleanTestUser() throws SQLException {
        ...
        String deleteSql = "DELETE FROM user WHERE userName = ? " +
                "OR userName LIKE 'bidder_signup_%' " +
                "OR userName LIKE 'seller_signup_%' " +
                "OR userName LIKE 'admin_signup_%'";

        try (Connection conn = MyDatabaseConfig.getConnection()) {
            //Tắt khóa ngoại
            try (PreparedStatement pst = conn.prepareStatement(disableFk)) {
                pst.execute();
            }

            //Xóa diện rộng toàn bộ tài khoản rác
            try (PreparedStatement pst = conn.prepareStatement(deleteSql)) {
                pst.setString(1, testUserName);
                int rowsDeleted = pst.executeUpdate();
                if (rowsDeleted > 0) {
                    System.out.println("-> Đã xoá " + rowsDeleted + " tài khoản kiểm thử rác khỏi DB.");
                }
            }

            //Bật lại khóa ngoại bảo vệ an toàn
            try (PreparedStatement pst = conn.prepareStatement(enableFk)) {
                pst.execute();
            }
        }
    }
